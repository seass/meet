...
class Excel {
     ...
     * @return Excel
     */
    public function renderData($data,$row=1) {
     ...
     * @return Excel
     */
    public function renderDataNew($data,$count=0) {
    	empty($this->activeSheet) && $this->createExcel();
    	$row = 1;
    	foreach ($data as $key => $vo) {
    		if (is_string($key)) {
    			$this->activeSheet->setCellValue($key, $this->_filterValue($vo, $key))->setShrinkToFit(true);
    			continue;
    		}
    		if (is_array($vo)) {
    			$col = 0;
    	...
    	$this->activeSheet->getStyle('A2:A1000')->getAlignment()->setWrapText(TRUE);//自动宽，超出宽度后自动折行
    	$this->activeSheet->getStyle('D2:D1000')->getAlignment()->setWrapText(TRUE);
    	$styleArray = array(
    			'borders' => array(
    					'allborders' => array(
    							'style' => "thin",//边框是粗的
//     							'style' => PHPExcel_Style_Border::BORDER_THIN,//细边框
    	...
    	$this->activeSheet->getStyle('A1:F'.$count)->applyFromArray($styleArray);//加边框，打印出来默认会有边框
    	return $this;
    }
     ...
     * 给OA 用了
     */
    public function renderDataOA() {
        empty($this->activeSheet) && $this->createExcel();
        $this->activeSheet->setCellValue('A1',$this->_filterValue('导入模版', ''));
