...
<body>
	...
	<div class="main-title cf">
		<h2><?php echo isset($info['id'])?'编辑':'新增';?>参会人员</h2>
	</div>
<form action="<?php echo U();?>" method="post" class="form-horizontal">
		<div class="form-item cf">
			...
			<div class="controls">
				<select name="store_id">
					...
					<?php $_result=get_store_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["store_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
		</div>
		<div class="form-item cf">
			...
			<div class="controls">
				<select name="classes_id">
					...
					<?php $_result=get_classes_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["classes_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
		</div>
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="realname" value="<?php echo ((isset($info["realname"]) && ($info["realname"] !== ""))?($info["realname"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="idcard" value="<?php echo ((isset($info["idcard"]) && ($info["idcard"] !== ""))?($info["idcard"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label"><span style="color:red;">*</span> 职务<span class="check-tips">（必填）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="position" value="<?php echo ((isset($info["position"]) && ($info["position"] !== ""))?($info["position"]):''); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label"> 头像照片<span class="check-tips"></span></label>
            <div class="controls">
                <input type="file" class="text " name="headimg" >
            </div>
        </div>
        <div class="form-item">
            <label class="item-label"> 考试成绩<span class="check-tips">（默认-1表示未参加考试）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="score" value="<?php echo ((isset($info["score"]) && ($info["score"] !== ""))?($info["score"]):'-1'); ?>">
            </div>
        </div>
        <div class="form-item">
        ...
        <div class="form-item cf">
			...
			<div class="controls">
				<label class="textarea input-large"><textarea name="food_req"><?php echo ($info["food_req"]); ?></textarea></label>
			</div>
		</div>
        <div class="form-item">
...
<script type="text/javascript" charset="utf-8">
	...
	Think.setValue('store_id',<?php echo ((isset($info["store_id"]) && ($info["store_id"] !== ""))?($info["store_id"]):0); ?>);
	Think.setValue('classes_id',<?php echo ((isset($info["classes_id"]) && ($info["classes_id"] !== ""))?($info["classes_id"]):0); ?>);
	Think.setValue('sex',<?php echo ((isset($info["sex"]) && ($info["sex"] !== ""))?($info["sex"]):0); ?>);
	Think.setValue('is_share',<?php echo ((isset($info["is_share"]) && ($info["is_share"] !== ""))?($info["is_share"]):0); ?>);
	//导航高亮
