...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Region/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Region/changeStatus',array('method'=>'resumeRegion'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('Region/changeStatus',array('method'=>'forbidRegion'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Region/changeStatus',array('method'=>'deleteRegion'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="region_name" class="search-input" value="<?php echo I('region_name');?>" placeholder="请输入大区名称">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<a href="<?php echo U('Region/edit?id='.$vo['id']);?>">编辑</a>
				<?php if($vo["status"] == 1): ?><a href="<?php echo U('Region/changeStatus?method=forbidRegion&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Region/changeStatus?method=resumeRegion&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('Region/changeStatus?method=deleteRegion&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		<td colspan="9" class="text-center"> Oh! 暂时还没有内容! </td><?php endif; ?>
	</tbody>
    </table>
	</div>
    <div class="page">
